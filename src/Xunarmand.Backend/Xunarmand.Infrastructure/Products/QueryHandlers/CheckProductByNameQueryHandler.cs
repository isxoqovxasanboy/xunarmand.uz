...
public class CheckProductByNameQueryHandler(IProductService service, IMapper mapper):IQueryHandler<CheckProductByNameQuery,string>
{
    public async Task<string> Handle(CheckProductByNameQuery request, CancellationToken cancellationToken)
    {
        var userFirstName = await service
                                  .Get(
                                      client => client.ProductName == request.ProductName,
